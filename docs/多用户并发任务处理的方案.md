> ⚠️ **本文档已被取代（2026-07-14，[ADR 0012](../../../docs/adr/0012-compute-scheduling-plane-argo.md)）。** 并发不再由本仓自己处理 —— 集群里 peptide-opt 的长驻 Deployment 已 `replicas: 0`，计算改由 **Argo Workflows** 承载：**每个任务一个 Workflow**（`fold→dock-score→redesign→report`，全 CPU），并发由 Argo controller 的 `parallelism: 8` 控制。下文"多容器 Worker 模式 ✅ 已实现"、`docker-manage.sh --workers 3`、`AsyncTaskProcessor` 轮询等**均为遗留方案**，仅适用于本地 docker-compose，不再是集群现状。
